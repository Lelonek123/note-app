...
export default function NotesBrowser() {
    ...
    const SearchHandler = (term) => {
        if (term == "") {
            ...
            let notes = backendNotes.filter((note) => note.tags.includes(term.substr(1).toLowerCase()));
            setNotes(notes);
        } else {
            let notes = backendNotes.filter((note) => note.name.toLowerCase().includes(term.toLowerCase()));
            setNotes(notes);
        }
    }
        ...
        <div className={`${style.wrapper}`}>
            ...
            <div className={`${style.notesContainer}`}>
                <div className={`row justify-content-around`}>
                    {loading ? <p>Loading</p> : notes.map((note) => <NoteCard {...note} key={note.name}/>)}
                </div>
            </div>
        </div>
    )
